...
async function submitReport() {
    ...
    try {
        ...
        if (response.ok) {
            const savedProb = await response.json();
            addProblem(savedProb); 
            updateLiveFeed(savedProb);
            
            reportsCreated++;
            document.getElementById('report-count').innerText = reportsCreated;
            showToast("Report saved to database! 🛰️");
        }
    } catch (error) {
        console.error("Backend offline, saving locally only.");
        addProblem({ ...probData, id: Date.now() }); 
    }
...
function addProblem(prob) {
    const confidence = (prob.trust_score !== undefined && prob.trust_score !== null) ? prob.trust_score : 70;
    ...
    const isOwner = (String(prob.owner_id) === String(myId));
    ...
    const barColor = confidence >= 70 ? "#eccc68" : "#ff4757";
    const popupContent = `
        <div id="popup-${prob.id}" style="text-align:center; min-width: 200px; font-family: 'Shrikhand', cursive;">
            ...
            <div style="margin: 10px 0; font-family: sans-serif;">
                ...
                <div class="confidence-bg">
                    <div class="confidence-fill" style="width: ${confidence}%; height:100%; background: ${barColor};"></div>
                </div>
            </div>
...
function handleVote(probId, isPositive) {
    ...
    const barColor = currentWidth >= 70 ? "#eccc68" : "#ff4757";
    ...
    fetch(`http://127.0.0.1:8000/api/reports/${probId}/`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ trust_score: currentWidth }) 
    })
    .then(response => {
        if (!response.ok) throw new Error("Server rejected update");
        console.log("Trust score saved! ✅");
    })
    ...
    if (currentWidth <= 25) {
        setTimeout(async () => {
            ...
            await deleteProblem(probId, true);
        }, 400);
    }
}
...
async function deleteProblem(id, isAuto = false) {
    
    if (isAuto || confirm("Are you sure you want to remove this report? 🦄")) {
        try {
            ...
            if (response.ok) {
                ...
                if (!isAuto) showToast("Report deleted! 💨");
            }
        } catch (error) {
...
async function loadReports() {
    try {
        ...
        console.log("Could not load reports from database.");
    }
}
